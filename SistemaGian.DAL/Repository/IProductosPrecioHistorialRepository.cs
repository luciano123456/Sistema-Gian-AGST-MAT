...
namespace SistemaGian.DAL.Repository
{
    public interface IProductosPrecioHistorialRepository<TEntityModel> where TEntityModel : class
    {
        ...
        Task<List<ProductosPreciosHistorial>> ObtenerUltimosPreciosProductoFecha(int idProducto, int idProveedor, int idCliente, DateTime FechaDesde, DateTime FechaHasta);
        Task<bool> Insertar(ProductosPreciosHistorial model);
        Task<bool> Actualizar(ProductosPreciosHistorial model);
        Task<bool> Eliminar(int id);
        Task<bool> RevertirPrecioAsync(int idHistorial, string tipo);
